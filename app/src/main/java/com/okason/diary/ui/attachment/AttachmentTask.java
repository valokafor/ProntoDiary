...
public class AttachmentTask extends AsyncTask<Void, Void, Attachment> {
    ...
    protected Attachment doInBackground(Void... params) {
        return StorageHelper.createAttachmentFromUri(ProntoDiaryApplication.getAppContext(), uri);
    }
    ...
    protected void onPostExecute(Attachment mAttachment) {
        if (isAlive()) {
            if (mAttachment != null) {
                EventBus.getDefault().post(new AttachingFileCompleteEvent(true, mAttachment));
            } else {
                EventBus.getDefault().post(new AttachingFileCompleteEvent(false, null));
            }
        } else {
            if (mAttachment != null) {
                StorageHelper.delete(ProntoDiaryApplication.getAppContext(), mAttachment.getUri());
            }
        }
    }

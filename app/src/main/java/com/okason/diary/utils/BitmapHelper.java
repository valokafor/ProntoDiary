...
public class BitmapHelper {
    ...
    public static Bitmap getBitmapFromAttachment(Context mContext, Attachment mAttachment, int width, int height) {
        ...
        if (Constants.MIME_TYPE_VIDEO.equals(mAttachment.getMime_type())) {
            ...
            path = StorageHelper.getRealPathFromURI(mContext, Uri.parse(mAttachment.getUriCloudPath()));
            // .. or directly from local directory otherwise
            if (path == null) {
                path = FileHelper.getPath(mContext, Uri.parse(mAttachment.getUriCloudPath()));
            }
		...
		// Image
            ...
            try {
                bmp = BitmapUtils.getThumbnail(mContext, Uri.parse(mAttachment.getUriCloudPath()), width, height);
	...
	public static Uri getThumbnailUri(Context mContext, Attachment mAttachment) {
		Uri uri = Uri.parse(mAttachment.getUriCloudPath());
